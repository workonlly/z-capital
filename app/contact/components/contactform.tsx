...
export default function ContactForm({ initialSubject = "", initialMessage = "" }: ContactFormProps) {
    ...
    const inputStyles =
        "w-full bg-[#111111] border border-white/10 rounded-sm px-6 py-4 mt-3 text-white placeholder:text-gray-500 focus:border-[#d4af37]/70 focus:bg-[#1a1a1a] focus:ring-1 focus:ring-[#d4af37]/50 outline-none transition-all duration-300 text-base min-h-[56px]"
    const labelStyles =
        "block text-[13px] font-bold text-[#d4af37]/90 mb-3 uppercase tracking-widest pl-1"
    const sectionHeaderStyles =
        "text-3xl sm:text-4xl font-medium text-white mb-14 flex items-center gap-6 mt-28 first:mt-0"

    return (
        <form
            ref={formRef}
            onSubmit={handleSubmit}
            className="w-full bg-[#0a0a0a]/80 py-10 !px-2 sm:py-12 sm:px-12 md:py-16 md:px-16 lg:py-20 lg:px-24 rounded-sm border border-white/10 shadow-2xl relative backdrop-blur-2xl overflow-hidden"
        >
            ...
            <div className="!mt-5  " ref={btnRef} style={{ opacity: 0 }}>
                <button
                    ...
                    className={`group w-full relative flex items-center justify-center gap-4 bg-[#d4af37] text-black py-6 px-10 rounded-sm text-xl font-bold uppercase tracking-widest overflow-hidden transition-all duration-500 ${isSubmitting
